/**
 * @file language resources
 * @author atom-yang
 */

export const resources = {
  en: {
    translation: {
      randomNumber: 'Random Number',
      lottery: 'Room Selection Lottery',
      weChatSweep: 'WeChat sweep',
      follow: 'Follow the aelf Offical Accounts',
      order: 'order',
      name: 'name',
      number: 'number',
      introduce:
        'Aelf is an efficient blockchain system that uses cutting-edge design and technology in the IT industry.',
      contactUs: 'contact us',
      aelf: 'Blockchain AELF',
      generationRandomNumber: 'The Generation Plan of Random Number',
      learnMore: 'Learn more',
      generationRandom: 'Random Number Generator',
      minNumber: 'Min',
      minPlaceholder: 'Please enter minimum',
      maxNumber: 'Max',
      maxPlaceholder: 'Please enter maximum',
      generate: 'generate',
      verify: 'Verification',
      random: 'Random Number of Current Round',
      currentBlockHeight: 'Block Height For Random Numbers Of Past Requests For This Query',
      requestBlockHeight: 'Block Height When Requesting Random Numbers',
      randomBlockHeight: 'The Block Height When Random Number Is Generated',
      playId: 'Block Transaction ID For Random Number Request',
      hash: 'Random Hash Value',
      result: 'Result',
      resultText: 'Click to Generate Random Numbers',
      inputError: 'Please enter the correct number',
      Building: 'Singaporean Splendid Building',
      lotSystem: 'The System of House Selection & Lot Number',
      lotNumber: 'Lot Number',
      commentThanks: 'Thank you for your attention and trust',
      specialNote: 'Special Note: ',
      commentNote:
        'The project is carried out by the Notary Public Office of the Housing Development Board of Singapore.'
        + 'The number is 100, and the number will be randomly selected. The number will be selected in order of this'
        + ' fairness! Shake the whole live broadcast to ensure that the sale of commercial housing is open and fair.',
      commentStart: 'Start Lot',
      beginLot: 'Household Selection Justice',
      beginLotBtn: 'Start',
      selectedResult: 'Lot Result',
      confirm: 'Confirm',
      request: 'Send Request To The Blockchain',
      generateid: 'Transaction Id Is Being Generated',
      confirmBlockHeight: 'The Target Block Height Is Being Determined',
      goheight: 'Going To The Target Block Height',
      producttionBlock: 'The Production Node Is Producing Blocks',
      complateHash: 'Calculating Hash Value',
      complateSign: 'Calculating Current Signature',
      generateRandom: 'Generating Random Numbers',
      gettingRandom: 'Getting Random Number',
      desc: 'Click on the above blue font to jump to the block browser to view detailed information'
    }
  },
  'zh-CN': {
    translation: {
      randomNumber: '随机数',
      lottery: '购房选房摇号',
      weChatSweep: '微信扫一扫',
      follow: '关注aelf社区公众号',
      order: '序号',
      name: '名字',
      number: '号码',
      introduce: 'aelf是一个真正达到商用标准, 采用了IT行业最前沿的设计和技术的高效区块链系统',
      contactUs: '联系我们',
      aelf: '区块链AELF',
      generationRandomNumber: '随机数生成方案',
      learnMore: '了解更多',
      generationRandom: '随机数生成器',
      minNumber: '最小值',
      minPlaceholder: '请输入最小值',
      maxNumber: '最大值',
      maxPlaceholder: '请输入最大值',
      currentBlockHeight: '本次查询过往申请的随机数的区块高度',
      requestBlockHeight: '申请随机数时的区块高度',
      randomBlockHeight: '生成随机数的区块高度',
      playId: '申请随机数的区块交易ID',
      generate: '生 成',
      result: '结 果',
      verify: '验 证',
      random: '本轮随机数',
      blockHeight: '区块高度',
      hash: '随机Hash值',
      resultText: '点击生成产生随机数',
      inputError: '请输入正确的数字',
      Building: '新加坡锦绣楼盘',
      lotSystem: '公正选房摇号系统',
      lotNumber: '启动摇号',
      commentThanks: '感谢大家的关注和信任',
      specialNote: '特别说明: ',
      commentNote: '本项目由新加坡建屋发展局公证处进行公正摇号，本次摇号总计100个号，将随机抽取5个号，将以本次公正摇出的顺序号码依次选房！摇号全程直播，确保商品住房销售公开公正公平。',
      commentStart: '开始摇号',
      beginLot: '选房公正摇号',
      beginLotBtn: '点击摇号',
      selectedResult: '摇号结果产生',
      confirm: '确定',
      request: '发送请求到区块链',
      generateid: '正在生成交易Id',
      confirmBlockHeight: '正在确定目标区块高度',
      goheight: '正在前往目标区块高度',
      producttionBlock: '生产节点正在生产区块',
      complateHash: '正在计算哈希值-in',
      complateSign: '正在计算当前签名-sign',
      generateRandom: '正在生成随机数',
      gettingRandom: '正在获取随机数',
      desc: '单击上面的蓝色字体以跳到块浏览器以查看详细信息'
    }
  }
};
